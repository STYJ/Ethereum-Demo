...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  function buyItem(uint sku)
    public
    payable
    forSale(sku) // check if item is for sale
    paidEnough(items[sku].price) // check if msg.value is >= price of item
    checkValue(sku) // refund excess
  {
    emit Sold(sku); // log event
    items[sku].seller.transfer(items[sku].price); // transfer all to seller
    items[sku].buyer = msg.sender; // update buyer
    items[sku].state = State.Sold; // update state
  }
  ...
  {
    emit Shipped(sku); // log event
    items[sku].state = State.Shipped; // update state
  }
